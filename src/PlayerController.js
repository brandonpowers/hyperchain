import {Scalar, Vector3, MeshBuilder} from "@babylonjs/core";
...
export class PlayerController {
  ...
  initPlayer() {
    ...
    this.playerMesh = new MeshBuilder.CreateBox("player_1", {
      width: 0.2,
      height: 0.2,
      depth: 0.2,
    }, this.scene);
  ...
  playerActions() {
    ...
    if (this.movementEnabled) {
      ...
      if (input.arrowup || input.w) {
        this.playerMoveForward(State.delta);
      }
      if (input.arrowdown || input.s) {
        this.playerMoveBackward(State.delta);
      }
    }
  ...
  playerMove() {
    ...
    this.playerMesh.position.y += this.y;
