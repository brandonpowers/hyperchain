...
export class Starfield {
  ...
  constructor(scene) {
    this.scene = scene;
    this.maxStars = eirlumeConfig.starField.numberofStars;
    this.stars = [];
    this.ratioOfBlinkers = eirlumeConfig.starField.ratioOfBlinkingStars;
    this.minY = -50;
    this.maxY = 150;
    this.minSize = eirlumeConfig.starField.minStarSize;
    this.maxSize = eirlumeConfig.starField.maxStarSize;
    ...
    this.speed = eirlumeConfig.starField.speed;

    // With actioncam enabled the starfield is rolling
    // at an angle. Speed it up to compensate.
    if (eirlumeConfig.actionCam) this.speed *= 2;

    if (eirlumeConfig.oldSchoolEffects.enabled && eirlumeConfig.oldSchoolEffects.blurIntensity > 0.2) {
  ...
  addStar() {
    let z = 10;
    if (eirlumeConfig.actionCam) z = 5;
