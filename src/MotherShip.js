...
export class MotherShip {
  ...
  constructor(gameAssets, scene) {
    ...
    this.interval = eirlumeConfig.motherShip.interval;
    this.fireRate = eirlumeConfig.motherShip.fireRate;
    this.hitsToKill = eirlumeConfig.motherShip.hitsToKill;
    this.velocity = eirlumeConfig.motherShip.velocity;
    this.scene = scene;
    this.bullets = [];
    this.enable();
  }
  ...
  startLoopArc() {
    ...
    this.mothershipLoop = this.scene.onBeforeRenderObservable.add(() => {
      ...
      if (eirlumeConfig.oldSchoolEffects.enabled) {
        ...
        this.mesh.rotate(Axis.Y, this.mesh.rotation.y + (eirlumeConfig.motherShip.rotateSpeed * State.delta), Space.LOCAL);
      }
      let pos = {
